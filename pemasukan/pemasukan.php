...
$stmt = mysqli_prepare($conn, "SELECT * FROM pemasukan WHERE user_id = ? ORDER BY tanggal DESC");
...
<head>
    ...
    <title>Data Pemasukan - Aplikasi Keuangan</title>
...
<body class="min-h-screen relative bg-slate-900">
        ...
        <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <span class="text-slate-400 text-sm hidden sm:block">
                    <span class="text-emerald-400 font-medium"><?php echo htmlspecialchars($_SESSION['nama']); ?></span>
                </span>
    ...
    <main class="relative z-10 max-w-6xl mx-auto px-6 py-10">
        ...
        <?php if (isset($_GET['success'])): ?>
            <div class="mb-5 px-4 py-3 rounded-lg bg-emerald-500/10 border border-emerald-400/30 text-emerald-300 text-sm flex items-center gap-2">
                ...
                <?php
                $msg = [
                    'tambah' => 'Data pemasukan berhasil ditambahkan.',
                    'edit'   => 'Data pemasukan berhasil diperbarui.',
                    'hapus'  => 'Data pemasukan berhasil dihapus.',
                ];
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl font-bold text-white">Data Pemasukan</h1>
                <p class="text-slate-400 text-sm mt-1">Riwayat pemasukan milik kamu</p>
            </div>
            <button type="button" onclick="document.getElementById('modalTambah').showModal()"
                class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-semibold transition shadow-lg shadow-emerald-500/30 w-fit">
                        ...
                        if (count($rows) === 0): ?>
                            <tr>
                                <td colspan="5" class="px-5 py-10 text-center text-slate-400 text-sm">
                                    Belum ada data pemasukan. Klik <span class="text-emerald-400 font-medium">Tambah Data</span> untuk mulai mencatat.
                                </td>
                            </tr>
                        <?php else:
                            foreach ($rows as $row): ?>
                            <tr class="border-b border-white/5 hover:bg-white/5 transition">
                                ...
                                <td class="px-5 py-3">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full bg-emerald-500/15 text-emerald-400 text-sm font-medium">
                                ...
                                <td class="px-5 py-3">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <a href="hapus_pemasukan.php?id=<?php echo $row['id']; ?>"
                            ...
                            <dialog id="modalEdit<?php echo $row['id']; ?>" class="backdrop:bg-slate-900/70 backdrop:backdrop-blur-sm rounded-2xl p-0 m-auto bg-transparent">
                                <div class="w-[90vw] max-w-md bg-slate-900/95 border border-white/20 rounded-2xl shadow-2xl p-6 sm:p-7">
                                    <div class="flex items-center justify-between mb-5">
                                        <h3 class="text-lg font-bold text-white">Edit Data Pemasukan</h3>
                                    ...
                                    <form method="POST" action="edit_pemasukan.php?id=<?php echo $row['id']; ?>" class="space-y-4">
                                        <div>
                                            ...
                                            <input type="date" name="tanggal" value="<?php echo htmlspecialchars($row['tanggal']); ?>" required
                                                class="w-full px-4 py-2.5 rounded-lg bg-white/5 border border-white/20 text-white focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition [color-scheme:dark]">
                                        </div>
                                        <div>
                                            ...
                                            <input type="text" name="keterangan" value="<?php echo htmlspecialchars($row['keterangan']); ?>" required
                                                class="w-full px-4 py-2.5 rounded-lg bg-white/5 border border-white/20 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition">
                                        </div>
                                        <div>
    ...
    <dialog id="modalTambah" class="backdrop:bg-slate-900/70 backdrop:backdrop-blur-sm rounded-2xl p-0 m-auto bg-transparent">
        <div class="w-[90vw] max-w-md bg-slate-900/95 border border-white/20 rounded-2xl shadow-2xl p-6 sm:p-7">
            <div class="flex items-center justify-between mb-5">
                <h3 class="text-lg font-bold text-white">Tambah Data Pemasukan</h3>
            ...
            <form method="POST" action="tambah_pemasukan.php" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-slate-200 mb-1.5">Tanggal</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-200 mb-1.5">Keterangan</label>
                    <input type="text" name="keterangan" placeholder="Contoh: Gaji bulan ini" required
                        class="w-full px-4 py-2.5 rounded-lg bg-white/5 border border-white/20 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-200 mb-1.5">Jumlah</label>
                    <input type="number" name="jumlah" placeholder="Contoh: 500000" required min="0"
                        class="w-full px-4 py-2.5 rounded-lg bg-white/5 border border-white/20 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition">
                </div>
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="submit" name="simpan"
                        class="flex-1 py-2.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-semibold transition shadow-lg shadow-emerald-500/30">
                        Simpan
                    </button>
                </div>
            </form>
